...
import (
	...
	"github.com/tv42/cliutil/strconvx"
)
...
func Parse(args interface{}, list []string) error {
	...
	for {
		...
		case reflect.Slice:
			...
			// append a zero value, then do pointer ops on it
			v := reflect.Append(field, reflect.Zero(field.Type().Elem()))
			field.Set(v)
			newItem := v.Index(v.Len() - 1).Addr()
			if setter, ok := newItem.Interface().(Setter); ok {
				if err := setter.Set(list[0]); err != nil {
					return err
				}
			} else {
				if err := strconvx.Parse(newItem.Interface(), list[0]); err != nil {
					return err
				}
			}
